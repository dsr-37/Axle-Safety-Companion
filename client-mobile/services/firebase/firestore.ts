...
export class FirestoreService {
  ...
  static async registerSupervisor(docId: string, userId: string, name: string, password?: string): Promise<void> {
    try {
  ...
  static async listTeamUsers(): Promise<UserProfile[]> {
    try {
  ...
  static async getHazardReportCountForUser(userId: string): Promise<number> {
    try {
      const q = query(collection(db, 'hazard_reports'), where('userId', '==', userId));
      const snapshot = await getDocs(q);
  ...
  static async resolveHazardReport(reportId: string, resolver?: { id?: string; name?: string }): Promise<void> {
    try {
      const docRef = doc(db, 'hazard_reports', reportId);
      await updateDoc(docRef, {
        status: 'resolved',
