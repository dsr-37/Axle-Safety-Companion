...
export default function SupervisorReports() {
  ...
  useEffect(() => {
    ...
    const qNew = query(collection(db, 'hazard_reports'), where('status', '==', 'pending'), orderBy('createdAt', 'asc'));
    const qLegacy = query(collection(db, 'hazardReports'), where('status', '==', 'pending'), orderBy('timestamp', 'asc'));
    ...
    }, (err) => console.warn('Reports listener (new) error:', err));
    ...
    }, (err) => console.warn('Reports listener (legacy) error:', err));

    return () => {
      try { unsubNew(); } catch (e) { /* noop */ }
      try { unsubLegacy(); } catch (e) { /* noop */ }
    };
  }, []);
              ...
              <ClayCard style={styles.card}>
                <View style={styles.cardInner}>
                  ...
                  <View style={styles.midCol}>
                    ...
                    <Text style={[styles.meta, { fontSize: 14 }]}>Reported by: {item.userName || item.userId} • {timeAgo(item.createdAt)}</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  midCol: { paddingHorizontal: 12 },
  rightCol: { width: 130, alignItems: 'center', paddingRight: 20 },
